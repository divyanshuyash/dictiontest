...
export default function RegistrationForm({ defaultGap = "", source = "KNOWN masterclass", score }: { defaultGap?: string; source?: string; score?: string }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="grid gap-5">
      ...
      <label className="form-field"><span>Biggest digital presence gap</span><select name="gap" required defaultValue={defaultGap}><option value="" disabled>Select the closest answer</option><option>People do not understand what I do</option><option>My audience is too broad</option><option>My content does not build authority</option><option>My website does not create trust</option><option>My platforms feel disconnected</option><option>Attention does not create opportunities</option><option>I do not have a follow-up system</option><option>I do not know what to build first</option></select></label>
      <label className="form-field"><span>Session</span><select name="session" required defaultValue=""><option value="" disabled>Choose a session preference</option><option value="next-live">Next live KNOWN session — schedule shared by email</option><option value="future">Notify me about a future session</option></select></label>
